...
// Schema de mise à jour (tous champs optionnels)
export const addressUpdateSchema = addressCreateSchema
  .omit({ type: true }) // Type non modifiable après création
  .partial()
...
export const addressValidation = {
  ...
  validateCountryPostal(postalCode: string, country: string): { isValid: boolean; error?: string } {
    ...
    return {
      isValid,
      error: isValid ? undefined : `Code postal invalide pour ${country}`
    }
  },
   ...
   */
  validateField(fieldName: string, value: any, context: Record<string, any>): { isValid: boolean; error?: string } {
    try {
      // Validation champ spécifique
      const fieldSchema = addressCreateSchema.shape[fieldName as keyof typeof addressCreateSchema.shape]
      
      if (!fieldSchema) {
        return { isValid: true }
      }
      
      // Parse avec contexte pour validations croisées
